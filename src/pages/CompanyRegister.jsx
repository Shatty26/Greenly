...
function CompanyRegister() {
  ...
  // Estados exclusivos para los campos solicitados
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [companyType, setCompanyType] = useState(""); // Menú desplegable
  const [companyCode, setCompanyCode] = useState(""); // Código creado manualmente por la empresa
  const [employeeCount, setEmployeeCount] = useState(""); // Nuevo estado para la cantidad de empleados
  ...
  const handleRegister = async (e) => {
    ...
    // 1. Validación de campos vacíos (incluyendo la cantidad de empleados)
    if (
      email.trim() === "" ||
      password.trim() === "" ||
      companyType === "" ||
    ...
    try {
      ...
      // 4. Guardar datos en Firestore en la colección 'empresas' usando el UID
      await setDoc(doc(db, "empresas", user.uid), {
        email: email,
        tipoEmpresa: companyType,
        codigoEmpresa: companyCode.trim(), 
        cantidadEmpleados: Number(employeeCount), // Guardamos el número ingresado de forma numérica
        role: "business_admin",           
        fecha: new Date().toLocaleString(),
        uid: user.uid,
      });

      // Limpiar inputs y redirigir
      setEmail("");
      setPassword("");
      setCompanyType("");
      setCompanyCode("");
